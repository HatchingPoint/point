...
- [ ] **Field alias resolution** — camelCase property access resolves to spaced record fields when unambiguous; diagnostics include fuzzy “did you mean” when not
- [ ] **Variant exhaustiveness** — `on Case` dispatch reports `missing-variant-case` for uncovered cases
- [ ] **`Maybe` presence narrowing** — `when <expr> present` / `when <expr> is none` narrows type in branch; improved `nullable-field-access` repair
