...
- Modules, imports, values, functions, named types, returns, conditionals, and expressions.
- Primitive types: `Text`, `Int`, `Float`, `Bool`, and `Void`.
- Generic lists with `List<T>`.
- Record literals assigned to named types.
...
- Formatting for canonical source.
...
```bash
bun run point:index
bun run point:explain src/point-core/examples/math.point point://core/Math/fn.userLabel
...
3. Grow the language with assignment, loops, errors/results, modules across files, async, and package resolution.
