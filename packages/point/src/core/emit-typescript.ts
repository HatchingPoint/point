...
export function emitPointCoreTypeScript(program: PointCoreProgram, sourcePath?: string): string {
	...
	if (needsDataLoad || needsStreamSubscribe || needsViewTabs || needsNavigationState || themeToggle || needsPointThemeToggle || needsFormSubmit) {
		lines.push('import * as React from "react";');
	}
	if (navigations.length > 0 || needsNavigationLinks || needsFormNavigate) {
		const routerImports = ["createBrowserRouter", "NavLink", "RouterProvider", "useParams"];
		if (needsFormNavigate) routerImports.push("useNavigate");
...
function emitFunction(
	...
	const viewHasExtras = Boolean(viewControls || viewNavigation || viewEach?.length || viewModal || viewTabs || viewToggleTheme);
	let bodyLines =
...
function emitViewWithExtrasBody(body: PointCoreStatement[], extras: ViewExtrasSpec): string[] {
	...
	const children = [...navLine, ...toggleLine, ...formLines, ...eachLines, ...tabsLine, ...modalLine, contentJsx];
...
function programNeedsAuthClient(program: PointCoreProgram): boolean {
	return program.declarations.some((declaration) => {
		...
		return Boolean(submit && (submit.withAuth || submit.saveTokenField));
	});
}
