...
export function emitPointCoreTypeScript(program: PointCoreProgram, sourcePath?: string): string {
	...
	if (navigations.length > 0 || needsNavigationLinks) {
		lines.push('import { createBrowserRouter, Link, RouterProvider, useParams } from "react-router-dom";');
	}
	if (needsDataLoad) {
	...
	for (const declaration of program.declarations) {
		...
		lines.push(...emitDeclaration(declaration, program, sourcePath), "");
	}
	...
	if (navigations.length > 0) {
		...
		for (const navigation of navigations) {
			lines.push(...emitClientNavigationRuntime(navigation, pages, records), "");
		}
	}
...
function emitDeclaration(
	...
	return emitFunction(declaration, program, sourcePath);
}
...
function emitFunction(
	...
	let bodyLines =
		declaration.semantic?.kind === "layout" && layoutSpec
			? emitLayoutBody(declaration.name, layoutSpec)
...
function emitLayoutBody(functionName: string, spec: PointSemanticLayoutSpec): string[] {
	...
	const lines = ["return (", `  <div className="point-layout point-layout-${toIdentifier(spec.name)}">`];
