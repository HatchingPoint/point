export function isSemanticPointSyntax(source: string): boolean {
	return source
		.split(/\r?\n/)
		.some((line) => /^(use|record|variant|calculation|rule|label|external|action|policy|guard|view|layout|navigation|page|middleware|stream route|route|workflow|pipeline|session|command|schedule|prompt)\s+/.test(line.trim()));
}
...
export function assertSemanticPointSource(source: string) {
	...
	for (const [index, line] of lines.entries()) {
		...
		if (/^(record|variant|calculation|rule|label|external|action|policy|guard|view|layout|navigation|page|middleware|stream route|route|workflow|pipeline|session|command|schedule|prompt)\s+/.test(trimmed)) hasSemanticDeclaration = true;
