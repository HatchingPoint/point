...
function missingApiKey(name: string): PointStdError {
	return { message: `Missing ${name} — set it with std.env before calling provider actions` };
}

function normalizeApiKey(apiKey: string | null | undefined): string | PointStdError {
...
function extractOpenaiCompletion(response: string): string | PointStdError {
	try {
...
function extractAnthropicCompletion(response: string): string | PointStdError {
	try {
		const parsed = JSON.parse(response) as {
...
function extractOpenaiStreamText(response: string): string | PointStdError {
	const chunks: string[] = [];
...
function extractAnthropicStreamText(response: string): string | PointStdError {
	const chunks: string[] = [];
	for (const line of response.split("\n")) {
		const trimmed = line.trim();
		if (!trimmed.startsWith("data:")) continue;
...
export async function anthropicComplete(
	...
	const normalized = normalizeApiKey(apiKey);
	if (typeof normalized !== "string") {
		return missingApiKey("ANTHROPIC_API_KEY");
	}
	const response = await postProviderRequest(
