...
export type PointRuntimeCryptoError = { message: string };
...
export function cryptoJwtVerify(token: string | null | undefined, secret: string): string | PointRuntimeCryptoError {
	const normalized = normalizeBearerToken(token);
	if (!normalized) return { message: "Missing JWT" };
