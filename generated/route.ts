...
export async function getUserRoute(id: string): Promise<Response | string> {
  return id;
}
...
export function createPointRouteFetchHandler() {
  ...
  if (req.method === "GET" && new RegExp("^\\/users\\/([^\\/]+)$").test(url.pathname)) {
    const match = url.pathname.match(new RegExp("^\\/users\\/([^\\/]+)$"));
    const handlerResult = getUserRoute(match[1]);
