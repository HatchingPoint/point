# Generated by Point. Do not edit directly.
# Point module: TimezoneDemo

from __future__ import annotations

import sys
from pathlib import Path as _PointPath
_point_here = _PointPath(__file__).resolve()
_point_module_dir = _point_here.parent
if str(_point_module_dir) not in sys.path:
    sys.path.insert(0, str(_point_module_dir))
_point_std_candidates = [_point_here.parents[1] / "packages" / "point" / "python_std"]
for _point_parent in _point_here.parents:
    _point_std_candidates.append(_point_parent / "node_modules" / "@hatchingpoint" / "point" / "python_std")
_point_std_root = next((candidate for candidate in _point_std_candidates if candidate.is_dir()), None)
if _point_std_root is not None and str(_point_std_root) not in sys.path:
    sys.path.insert(0, str(_point_std_root))

from typing import TypedDict

from time import instantNowRaw, parseInstantRaw, formatInstantRaw, formatInstantInTimezoneRaw, timeNow, sleepMilliseconds, formatTime, durationFromSecondsRaw, durationToSecondsRaw, durationFromMinutesRaw, instantNowValue, formatInstantLabel, formatInstantInTimezoneLabel

class Meeting(TypedDict):
    title: str
    at: str

def launchMeeting() -> Meeting:
    return {"title": "Product launch", "at": instantNowValue()}

def meetingInTimezoneLabel(meeting: Meeting, timezone: str) -> str:
    return formatInstantInTimezoneLabel(meeting["at"], timezone)

async def timezoneDemoCommand() -> str:
    return meetingInTimezoneLabel(launchMeeting(), "Asia/Tokyo")

if __name__ == "__main__":
    import asyncio
    __point_result = asyncio.run(timezoneDemoCommand())
    if __point_result is not None:
        print(__point_result if isinstance(__point_result, str) else __import__("json").dumps(__point_result))
